...
class _EjecucionesScreenState extends State<numero_repe> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea( // Asegura que el contenido no se superponga a la barra de estado del dispositivo
        child: Column( // Usa una columna para organizar los widgets verticalmente
          ...
          children: [
            ...
            Container(
              ...
              child: const Row( // Usa una fila para organizar los elementos horizontalmente
                children: [
                  /*Image.asset(
            ...
            Padding(
              ...
              child: Column( // Columna para organizar los contenedores verticalmente
                children: [
                  _buildInfoRow('Rep/Carga', 'Tipo de Serie', 'Tipo de Serie'), // Fila de información
                  const SizedBox(height: 16), // Espacio vertical
                  _buildNumberControl('Número de serie', numeroSerie, (value) { // Control numérico para número de serie
                    setState(() { // Actualiza el estado del widget
                      numeroSerie = value; // Asigna el nuevo valor
                    });
                  }),
                  const SizedBox(height: 16), // Espacio vertical
                  _buildNumberControl('Número de repeticiones', numeroRepeticiones, (value) { // Control numérico para repeticiones
                    setState(() {
                      numeroRepeticiones = value;
                    });
                  }),
                  const SizedBox(height: 16), // Espacio vertical
                  _buildNumberControl('Tiempo de repetición (segundos)', tiempoRepeticion, (value) { // Control numérico para tiempo
                    setState(() {
                      tiempoRepeticion = value;
                    });
                  }),
                  const SizedBox(height: 16), // Espacio vertical
                  _buildNumberControl('Carga', carga, (value) { // Control numérico para carga (permite decimales)
                    setState(() {
                      carga = value;
                    });
                  }, isDouble: true), // Para carga, permite decimales
                  const SizedBox(height: 16), // Espacio vertical
                  const Row( // Fila para el asterisco y el texto "carga"
                    mainAxisAlignment: MainAxisAlignment.spaceBetween, // Distribuye los elementos en el espacio disponible
            ...
            Container(
              color: const Color.fromARGB(255, 43, 159, 17), // Color de fondo verde oscuro
              padding: const EdgeInsets.all(8.0), // Padding interno
              child: Row( // Fila para organizar los elementos horizontalmente
                mainAxisAlignment: MainAxisAlignment.spaceAround, // Distribuye los elementos en el espacio disponible
                children: [
                  IconButton( // Botón para reproducir
                    onPressed: () {
                      setState(() {
                    ...
                    icon: const Icon(Icons.play_arrow, color: Colors.white, size: 30), // Icono de "play"
                  ),
                  IconButton( // Botón para pausar
                    onPressed: () {
                      setState(() {
                    ...
                    icon: const Icon(Icons.pause, color: Colors.white, size: 30), // Icono de "pause"
                  ),
                  IconButton( // Botón para detener
                    onPressed: () {
                      setState(() {
  ...
  Widget _buildNumberControl(String label, dynamic value, Function(dynamic) onChanged, {bool isDouble = false}) {
    return Container(
      padding: const EdgeInsets.all(16.0), // Padding interno
      decoration: BoxDecoration(
        color: Colors.grey[900], // Color de fondo gris oscuro
      ...
      child: Row( // Fila para organizar los elementos horizontalmente
        ...
        children: [
          ...
          Row( // Fila para los botones y el valor
            children: [
              IconButton( // Botón para decrementar
                icon: const Icon(Icons.remove, color: Colors.white),
                onPressed: () {
                  onChanged(isDouble ? value - 0.5 : value - 1); // Decrementa el valor (0.5 si es double)
                },
              ),
              Text(value.toString(), style: const TextStyle(color: Colors.white)), // Valor actual
              IconButton( // Botón para incrementar
                icon: const Icon(Icons.add, color: Colors.white),
                onPressed: () {
                  onChanged(isDouble ? value + 0.5 : value + 1); // Incrementa el valor (0.5 si es double)
                },
              ),
            ],
          ),
